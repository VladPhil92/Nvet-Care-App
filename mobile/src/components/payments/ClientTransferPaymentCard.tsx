...
const BANCOLCOLOMBIA_KEY = '1047444344'
const QR_SOURCE = require('../../../assets/payments/bancolombia-bre-b-qr.jpeg')
...
export default function ClientTransferPaymentCard({
  ...
  const helperText = useMemo(() => {
    ...
    return 'Escanea el QR o usa la llave Bancolombia. Después de transferir, sube el comprobante para validación administrativa.'
    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={styles.helper}>
          Inicia el pago para visualizar el QR oficial dentro de este chat.
        </Text>
        <Pressable
        ...
        >
          {busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryText}>Proceder a pagar</Text>}
        </Pressable>
      </View>
    )
  }
      ...
      {showQr ? (
        <>
          <Image
            ...
            style={styles.qr}
            accessible
            accessibilityLabel="QR de pago Bancolombia Bre-B"
          />
          <View style={styles.keyBox}>
            <Text style={styles.keyLabel}>Llave</Text>
            <Text selectable style={styles.keyValue}>{BANCOLCOLOMBIA_KEY}</Text>
          </View>
        </>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  supportText: { color: UI_COLORS.error, fontWeight: '800', fontSize: 12, textAlign: 'center' },
})
