...
import { BETA_EVIDENCE_GATES, BetaEvidenceGate } from "./beta-evidence.constants";
import { BetaEvidenceActor, BetaEvidenceService } from "./beta-evidence.service";
...
export class CartagenaLaunchOperationsService {
  ...
  async getSnapshot() {
    ...
    if (!observationTracked) blockers.push("ACTIVE_BETA_OBSERVATION_NOT_TRACKED");
    ...
    const checklist = [
      ...
      },
      {
        ...
        label: "Active beta has a durable observation record",
  ...
  async startObservation(dto: StartLaunchObservationDto, actor: BetaEvidenceActor) {
    ...
    if (["ACTIVE", "ELIGIBLE_TO_CLOSE"].includes(current.state)) {
      throw new ConflictException("A Cartagena beta observation window is already active.");
    }
    if (
  ...
  async closeObservation(dto: CloseLaunchObservationDto, actor: BetaEvidenceActor) {
    const [readiness, current] = await Promise.all([
      this.launchReadiness.getSnapshot(),
      this.getObservationStatus(),
    ]);
  ...
  async abortObservation(dto: AbortLaunchObservationDto, actor: BetaEvidenceActor) {
    const current = await this.getObservationStatus();
    if (
      !current.observationId ||
      !["ACTIVE", "ELIGIBLE_TO_CLOSE"].includes(current.state)
    ) {
      throw new ConflictException("There is no active observation window to abort.");
    }
  ...
  async getObservationStatus() {
    ...
    const conflicted = observations.filter((item) => item.conflictReasons.length > 0);
  ...
  private deriveObservation(events: ObservationEvent[]) {
    ...
    for (const event of sorted) {
      if (event.metadata.eventType === "STARTED") {
        if (startedAt) conflictReasons.push("DUPLICATE_START_EVENT");
        else startedAt = event.createdAt.toISOString();
      } else if (event.metadata.eventType === "CLOSED") {
        if (!startedAt) conflictReasons.push("CLOSE_WITHOUT_START");
        if (closedAt || abortedAt) conflictReasons.push("DUPLICATE_TERMINAL_EVENT");
    ...
    else if (daysElapsed >= MINIMUM_OBSERVATION_DAYS) state = "ELIGIBLE_TO_CLOSE";
  ...
  private buildExpiryItems(input: {
    ...
    for (const gate of BETA_EVIDENCE_GATES) {
      ...
      const selected = candidates.find((item) => item.status === "APPROVED") ?? candidates[0];
  ...
  private buildExpiryWatch(items: ReturnType<CartagenaLaunchOperationsService["buildExpiryItems"]>) {
    ...
    const state = items.reduce<ExpiryState>(
      (worst, item) => (weight[item.state] > weight[worst] ? item.state : worst),
      "NON_EXPIRING",
    );
  ...
  private resolveOperatorAction(input: {
    ...
    observation: Awaited<ReturnType<CartagenaLaunchOperationsService["getObservationStatus"]>>;
  ...
  private async appendObservationEvent(
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        severity:
          metadata.eventType === "ABORTED" ? AuditSeverity.WARN : AuditSeverity.INFO,
