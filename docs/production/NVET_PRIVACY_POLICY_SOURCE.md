...
De forma separada a esa telemetría propia, la versión móvil de producción puede utilizar Sentry como proveedor de diagnóstico de errores. Cuando ocurre un fallo, Sentry puede recibir el tipo y mensaje de la excepción, un stack trace del cliente, la versión/entorno de la aplicación y breadcrumbs técnicos sanitizados. La configuración desactiva el envío predeterminado de PII, elimina cuerpos de solicitudes, cookies y cabeceras de autorización/API, y mantiene deshabilitado el tracing de rendimiento en la primera versión comercial para evitar transmitir URLs o spans que pudieran contener parámetros sensibles como ubicación precisa. La relación de tratamiento, conservación y clasificación de Sentry debe verificarse contra la configuración y contrato reales de producción antes de publicar la declaración final de Google Play.
...
El servicio puede utilizar proveedores o componentes de infraestructura, almacenamiento, mapas, comunicaciones, pagos, monitoreo, inteligencia artificial e identidad. La aplicación móvil puede utilizar Sentry específicamente como encargado/proveedor de diagnóstico de errores, sujeto a la configuración, retención y relación contractual vigentes en producción. CTG One participa en el flujo opcional de federación de identidad cuando el usuario elige ese método de acceso. La clasificación jurídica y de Google Play de ese procesamiento debe corresponder con la relación real de producción y no se presume como “sharing” o “no sharing” únicamente a partir del código fuente.
